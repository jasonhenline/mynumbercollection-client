...
export default function NewGrantCarouselView(props: NewGrantCarouselViewProps) {
    ...
    const sortedNumbers = props.newNumbers
        .map((n) => n.number)
    ...
    const rightColor =
        cardIndex === sortedNumbers.length - 1
            ? theme.colors.onSurfaceDisabled
            : theme.colors.onSurface;
    const arrowSize = 36;
    ...
    return (
        <View style={{ alignItems: "center", gap: 20 }}>
            ...
            <View style={{ flexDirection: "row", alignItems: "center" }}>
                ...
                <CardView
                    number={sortedNumbers[cardIndex]}
                    isNew={!!numberToNewMap.get(sortedNumbers[cardIndex])}
                ></CardView>
                <TouchableRipple
                    onPress={() => setCardIndex(cardIndex + 1)}
                    disabled={cardIndex === sortedNumbers.length - 1}
                >
                    <AntDesign
                        name="right"
                        color={rightColor}
            ...
            <Text variant="headlineMedium">
                {cardIndex + 1}/{sortedNumbers.length}
            </Text>
